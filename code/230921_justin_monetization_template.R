...
#load data
iban <- read.csv('data/monetization_raw/justin_iban_230918_2023_09_18.csv') %>%
  dplyr::mutate(type = 'iban',
                date = as.Date(post_date, '%d/%m/%y'))
amazon <- read.csv('data/monetization_raw/justin_amazon_230918_2023_09_18.csv')%>%
  dplyr::mutate(type = 'amazon',
                date = as.Date(post_date, '%d/%m/%y'))
crowdfunding <- read.csv('data/monetization_raw/justin_monetization_crowdfunding_230918_2023_09_18.csv')%>%
  dplyr::mutate(type = 'crowibanunding',
                date = as.Date(post_date, '%d/%m/%y'))
crypto <- read.csv('data/monetization_raw/justin_monetization_crypto_230918_2023_09_18.csv')%>%
  dplyr::mutate(type = 'crypto',
                date = as.Date(post_date, '%d/%m/%y'))
cashapps <- read.csv('data/monetization_raw/justin_monetization_secondary_cashapps_230918_2023_09_19.csv')%>%
  dplyr::mutate(type = 'cashapps',
                date = as.Date(post_date, '%d/%m/%y'))
paypal <- read.csv('data/monetization_raw/justin_monetization_cashapps_paypal1_230918_2023_09_19.csv') %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal2_230918_2023_09_19.csv')) %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal3_230918_2023_09_19.csv')) %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal4_230918_2023_09_19.csv')) %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal5_230918_2023_09_19.csv')) %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal6_230918_2023_09_19.csv')) %>%
  dplyr::bind_rows(read.csv('data/monetization_raw/justin_monetization_cashapps_paypal7_230918_2023_09_19.csv'))%>%
...
climate_posts_monetization <- climate_posts %>%
  mutate(iban = ifelse(post_id %in% iban$post_id, 1, 0),
         amazon = ifelse(post_id %in% amazon$post_id, 1, 0),
         crowdfunding = ifelse(post_id %in% crowdfunding$post_id, 1, 0),
         crypto = ifelse(post_id %in% crypto$post_id, 1, 0),
         paypal = ifelse(post_id %in% paypal$post_id, 1, 0)) %>%
  mutate(monetization_count = select(., iban, amazon, crowdfunding, crypto, paypal) %>% rowSums(),
         monetization_binary = ifelse(monetization_count > 0, 1, 0))

write.csv(climate_posts_monetization, 'data/231130_posts_all_vars.csv')
...
complete_climate_monetization <- complete %>%
  filter(is_climate_post == 1)
write.csv(complete_climate_monetization, 'results/monetization/complete_climate.csv')
### IBAN ###
#Extract IBANs
iban <- iban %>%
