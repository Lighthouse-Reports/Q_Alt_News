...
setwd("/Users/justin-casimirbraun/Documents/Lighthouse Reports/Q Europe/Alt Media/analysis") #CHECK: specify working dir
...
posts <- posts %>%
  ...
  dplyr::mutate(sum_topics = fifteen_minutes + climate_lockdown) %>%
  #This creates separate categories for posts with overlapping topics. 
  #It would also be conceivable to just duplicate the row when multiple topics are concerned
  #CHECK: add additional cols for additional topics
  dplyr::mutate(climate_lockdown = ifelse(climate_lockdown == 1, 'climate_lockdown ', ''),
